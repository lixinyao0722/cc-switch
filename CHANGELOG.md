...
- **ModelHub R21 Chained History Recovery**: Historical and side-task requests now chain the cross-resource item-ID fallback into the encrypted-reasoning fallback when ModelHub exposes `invalid_encrypted_content` only after the IDs are removed. Each exact compatibility repair still runs at most once, preserves tool `call_id` relationships and visible content, and does not broaden retries for ordinary HTTP 400 responses.
...
- **Native Context Continuation and Compaction**: The ModelHub profile enables Codex `remote_compaction_v2`. CC Switch reuses `previous_response_id` only when the exact acknowledged input/output prefix and stable request options match; forks, concurrent turns, expired checkpoints, restarts, and cursor errors fail closed to full history. ModelHub's opaque compact output remains owned by Codex and local transcripts are never deleted.
- **429 Admission Control**: Provider/model-scoped weighted admission serializes prompts estimated at 100k tokens or above while retaining four slots for smaller requests. ModelHub capacity error `-2004` is retried at most once; metadata helpers remain single-attempt and existing shared cooldown behavior remains active.

## [3.20.0] - 2026-08-18

- **ModelHub R22 Release Contract**: Align the immutable one-click installer with CC Switch 3.20.0 using tag `modelhub-installer-20260828-r22` and asset `CC-Switch-ModelHub-3.20.0-arm64.app.zip`, while retaining the R21 routing, context recovery, and 429 admission behavior.
...
- **ModelHub Codex Proxy Resilience**: ModelHub Codex requests now repair empty namespace descriptions before forwarding, fail fast on HTTP 400/401/403 client errors, and enforce a 10-minute SSE lifetime with useful-progress detection even when failover is disabled. The installer defaults to high reasoning effort and reduces same-provider 429 retries from ten to three with bounded jitter, while preserving valid `Retry-After` guidance.
- **ModelHub Codex Activity Summary Quota Guard**: ModelHub providers can now opt into a narrow local guard for Codex Desktop's fixed `gpt-5.6-luna` task-activity summary request. The R9 installer enables it by default because the shared ModelHub credential has no Luna permission: matching requests fail locally with a non-retryable 400 instead of reaching ModelHub repeatedly, while ordinary Luna requests, other prompts, other providers and Sol traffic are unchanged.
- **ModelHub Codex Metadata and Encrypted-Reasoning Resilience**: The R10 installer maps only Codex Desktop's recognized title and description metadata requests from unauthorized Luna to Sol while activity summaries remain locally blocked and ordinary Luna traffic stays unchanged. After ModelHub proves a client-provided session cannot validate historical encrypted reasoning, CC Switch remembers that Provider/session in process and pre-cleans later requests before their first upstream attempt, avoiding repeated full-request replays.
- **Configurable ModelHub Compatibility Policies**: The R11 installer exposes the R10 metadata model, activity-summary handling, encrypted-reasoning learning, session adapter, and 429 policy in the Codex Provider UI. Activity summaries default to precise Luna-to-Sol mapping, suppress identical five-second replays, and never inherit main-request 429 retries. Unknown Luna helper requests remain unchanged while privacy-safe prompt fingerprints and structural counts provide evidence for future exact classification.
- **ModelHub R12 Auxiliary and Rate-Limit Resilience**: Activity-summary Skill-selection helpers are precisely mapped to the configured metadata model and all explicit metadata/helper requests bypass 429 retries. Invalid encrypted reasoning recovery now strips only the unusable ciphertext and preserves visible summaries/content. Main ModelHub requests default to one same-provider 429 recovery attempt behind a shared Provider cooldown with a single recovery probe; the installer sets Codex request/stream retries to 2/3 and removes the unsupported `retry_429` key.
- **ModelHub R13 Skill Selection and Credential Reuse**: Every protocol-valid Codex dynamic Skill-selection helper now maps to the configured metadata model, covering main-task, title, and activity-summary flows without broadly rewriting ordinary Luna requests. The installer detects an inherited `MODELHUB_AK`, asks whether to reuse it by default, permits hidden replacement input, and synchronizes the selected credential across Keychain, the ModelHub Provider, and launchd with transactional rollback.
- **ModelHub R14 Mobile Session Routing**: The installer now atomically maintains `/etc/codex/managed_config.toml`, defaulting ordinary sessions to ModelHub while redirecting the built-in `openai` base URL through CC Switch so mobile-created sessions cannot bypass the local proxy. System writes stage through `/private/var/tmp`, preserve unrelated managed policy, and fully roll back existing or newly created files. R14 also lowers automatic compaction to 500,000 tokens, raises main-request same-provider 429 recovery to two retries, and preserves the Codex App `feat/` Git branch prefix.
- **ModelHub R15 Catalog and Codex Setup**: The packaged GPT-5.5 catalog and Rust template now advertise a 1,050,000-token maximum at 100% effective context while the approved GPT-5.6 values remain unchanged. Golden Codex configuration enables the context-usage indicator, prevents system sleep during tasks, and enables the bundled Computer Use plugin without duplicating its MCP declaration. Before the first required `sudo` validation, the installer now explains that macOS expects the current login user's administrator password, not `MODELHUB_AK`, and that terminal password input is not echoed.
- **ModelHub R16 Codex Runtime Baseline**: The installer now pins the dated `gpt-5.5-2026-04-24` review model, sends the live Golden Codex configuration directly through `127.0.0.1:15721`, and exposes only high, extra-high, and maximum reasoning choices. The public Golden config includes the approved Computer Use MCP and bundled Node REPL runtime entries while continuing to exclude editor preferences, Marketplace cache metadata, credentials, and machine-specific user paths. Golden database snapshots normalize the live proxy URL back to the real ModelHub upstream so requests cannot loop into CC Switch. The installer now also verifies that merge-mode installs retain the top-level ModelHub provider selection and preloads an `OpenAI Official` entry for one-click switching between ModelHub and the user's ChatGPT subscription while CC Switch is running.
