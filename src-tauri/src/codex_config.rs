...
fn set_codex_experimental_bearer_token_with_policy(
    config_text: &str,
    token: &str,
    allow_env_key: bool,
) -> Result<String, AppError> {
    ...
    if let Some(provider_table) = doc
        ...
        .and_then(|item| item.as_table_like_mut())
    {
        let only_env_key_short_circuit = allow_env_key
            && provider_table.get("env_key").is_some()
            && provider_table.get("auth").is_none()
...
fn set_codex_experimental_bearer_token(config_text: &str, token: &str) -> Result<String, AppError> {
    set_codex_experimental_bearer_token_with_policy(config_text, token, false)
}

/// Keep a provider backup self-contained when its canonical configuration
/// names an environment variable but the persisted provider row also carries
/// the credential. The normal live-write path still prefers `env_key` and avoids duplicating
/// its secret; a takeover backup must survive without that process
/// environment after restart, so it additionally stores the scoped bearer.
pub(crate) fn set_codex_backup_bearer_token(
    config_text: &str,
    token: &str,
) -> Result<String, AppError> {
    set_codex_experimental_bearer_token_with_policy(config_text, token, true)
}
