...
"#
    ...
    async fn update_current_claude_desktop_provider_syncs_profile_when_proxy_takeover_is_active() {
        ...
        let proxy_info = state
            ...
            .await
            .expect("start proxy service");
        assert_ne!(proxy_info.port, 0, "OS should assign a concrete port");
...
"#
    ...
    fn codex_switch_live_failure_restores_both_current_sources_and_live_files() {
        with_test_home(|state, _| {
            ...
            write_live_with_common_config_for_state(state, &AppType::Codex, &provider_a).unwrap();
    ...
    async fn routed_switch_rolls_back_partial_takeover_disable_state() {
        ...
        write_live_with_common_config_for_state(&state, &AppType::Codex, &modelhub).unwrap();
        state
            .proxy_service
...
impl ProviderService {
    fn write_switch_live(
        ...
        Self::write_preflighted_or_current_live(state, app_type, provider, preflighted_provider)
    }
    ...
    fn switch_normal(
        ...
        if use_managed_codex_transaction {
            ...
            let commit_result = (|| -> Result<(), AppError> {
                ...
                Self::write_switch_live(state, &app_type, provider, preflighted_provider.as_ref())?;
                if !app_type.is_additive_mode() {
