...
impl ProxyService {
    ...
    pub(crate) async fn update_live_backup_from_provider_inner(
        ...
        if matches!(app_type_enum, AppType::Codex) {
            ...
            let backup_token = effective_settings
                .get("auth")
            ...
            // A takeover backup must remain recoverable after the app or shell
            // restarts. ModelHub stores its canonical key in the provider row
            // while naming MODELHUB_AK through env_key; preserve that key as a
            // provider-scoped bearer in the backup without changing the normal
            // live-write policy for env-backed providers.
            if !is_codex_official {
                let config_text = effective_settings
                    .get("config")
                    .and_then(Value::as_str)
                    .map(str::to_string);
                if let (Some(token), Some(config_text)) = (backup_token, config_text) {
                    let backup_config =
                        crate::codex_config::set_codex_backup_bearer_token(&config_text, &token)
                            .map_err(|error| format!("更新 Codex 备份凭据失败: {error}"))?;
                    if let Some(root) = effective_settings.as_object_mut() {
                        root.insert("config".to_string(), Value::String(backup_config));
                    }
                }
            }
