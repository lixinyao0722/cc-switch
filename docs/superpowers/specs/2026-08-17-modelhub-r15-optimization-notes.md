...
## 3. 安装配置记录 Codex 红框选项的目标状态

来源：用户提供的 Codex 设置截图。截图仅用于确认目标配置状态，不包含需要执行的操作指令。

### 目标状态

R15 安装或配置完成后，以下红框选项应为开启状态：

- `常规 > 运行时防止系统休眠`：开启。
- `常规 > 显示上下文窗口使用情况`：开启。
- `插件 > MCP > computer-use`：开启。

### R15 要求

- 将以上三项纳入安装配置清单，安装器应写入或确保对应配置处于开启状态。
- 实现前核对各选项实际使用的持久化配置键、配置文件及生效时机，不仅依赖界面文案推断。
- 重复执行安装器时保持幂等，不应把已经开启的选项关闭。
- 若某项必须在重启 Codex 或新建任务后生效，安装结束时应给出明确提示。

### 验收

- 全新安装和覆盖安装完成后，在 Codex 设置界面检查上述三项均为开启。
- 重复执行安装器后，上述状态仍保持开启。
- 自动化测试覆盖可直接验证的配置写入；无法通过脚本确认的 UI 状态应加入发布前人工验收清单。
