...
3. 保持 R11 Provider 配置和 UI 契约不变，发布安装器 `modelhub-installer-20260816-r12`。
...
- Golden DB 和 Provider UI 默认值保持 R11：session adapter 开启、活动摘要 `map`、元数据模型 `gpt-5.6-sol`、加密推理学习开启、主请求 429 最多重试 3 次。
