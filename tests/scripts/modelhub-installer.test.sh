...
test_preflight_verifies_all_release_checksums() {
  ...
  mkdir -p "$case_dir"
  printf 'installer\n' >"$case_dir/install.sh"
  printf 'app\n' >"$case_dir/CC-Switch-ModelHub-3.19.4-arm64.app.zip"
  printf 'resources\n' >"$case_dir/modelhub-installer-resources.tar.gz"
  (
    cd "$case_dir"
    shasum -a 256 \
      install.sh \
      CC-Switch-ModelHub-3.19.4-arm64.app.zip \
...
test_preflight_downloads_from_immutable_release_tag() {
  ...
  printf 'app\n' >"$remote_dir/CC-Switch-ModelHub-3.19.4-arm64.app.zip"
  printf 'resources\n' >"$remote_dir/modelhub-installer-resources.tar.gz"
  printf 'checksums\n' >"$remote_dir/SHA256SUMS.txt"
  assert_equals "$RELEASE_TAG" 'modelhub-installer-20260827-r18'
  printf '%s\n' \
    '#!/bin/bash' \
    'set -euo pipefail' \
    'output=""' \
    ...
    '  esac' \
    'done' \
    '[[ "$url" == *"/releases/download/modelhub-installer-20260827-r18/"* ]]' \
  ...
  assert_contains "$output_dir/CC-Switch-ModelHub-3.19.4-arm64.app.zip" 'app'
...
create_fake_app_zip() {
  ...
  COPYFILE_DISABLE=1 /usr/bin/ditto -c -k --keepParent "$app_dir" "$case_dir/assets/CC-Switch-ModelHub-3.19.4-arm64.app.zip"
}
...
test_r18_release_contract_and_documentation() {
  assert_contains "$INSTALLER" "readonly RELEASE_TAG='modelhub-installer-20260827-r18'"
  assert_contains "$INSTALLER" '下载并校验 R18 安装器、CC Switch 和配置资源'
  ...
  assert_contains "$MODELHUB_GUIDE" '是否使用 R18 标准配置完整覆盖？[y/N]'
  ...
  assert_contains "$CHANGELOG_FILE" 'ModelHub R18'
}
...
test_package_builds_exact_allowlisted_release_assets() {
  ...
  assert_contains \
    ...
    "readonly RELEASE_TAG='modelhub-installer-20260827-r18'"
...
test_package_rejects_nonempty_output_directory() {
  ...
  [[ ! -e "$output_dir/CC-Switch-ModelHub-3.19.4-arm64.app.zip" ]] \
...
test_release_smoke_installs_repeats_and_rolls_back_packaged_assets() {
  ...
  if [[ -n "${CC_SWITCH_RELEASE_SMOKE_ASSET_DIR:-}" ]]; then
    ...
    run_packager \
      ...
      "$case_dir/assets/CC-Switch-ModelHub-3.19.4-arm64.app.zip" \
      "$asset_dir"
  fi
...
run_test "R18 release contract and documentation" test_r18_release_contract_and_documentation
...
run_test "R18 preflight downloads from immutable release tag" test_preflight_downloads_from_immutable_release_tag
...
run_test "R18 package builds exact allowlisted release assets" test_package_builds_exact_allowlisted_release_assets
