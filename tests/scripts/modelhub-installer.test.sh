...
test_preflight_downloads_from_immutable_release_tag() {
  ...
  assert_equals "$RELEASE_TAG" 'modelhub-installer-20260727-r2'
  printf '%s\n' \
    '#!/bin/bash' \
    'set -euo pipefail' \
    'output=""' \
    ...
    '  esac' \
    'done' \
    '[[ "$url" == *"/releases/download/modelhub-installer-20260727-r2/"* ]]' \
...
test_package_builds_exact_allowlisted_release_assets() {
  ...
  assert_contains \
    ...
    "readonly RELEASE_TAG='modelhub-installer-20260727-r2'"
